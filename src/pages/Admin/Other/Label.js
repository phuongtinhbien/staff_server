...
const UPDATE_WASHER_STATUS = gql `mutation updateOptionList($id: BigFloat!, $status: String!, $currUser: BigFloat!) {
  updateLabelById(input: {id: $id, labelPatch: {status: $status, updateBy: $currUser}}) {
    label {
      nodeId
      id
      labelName
  ...
  render() {
    ...
    function onAfterInsertRow(row) {
      ...
      client.mutate({mutation: ADD_OPTION,
        variables:{keyName: row.labelName, currUser: CURRENT_USER.currentUser.id, status: row.status }}).then(
          data => {
            console.log(data)
              if (data){

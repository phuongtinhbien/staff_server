...
const UPDATE_WASHER_STATUS = gql `mutation updateOptionList($id: BigFloat!, $status: String!, $currUser: BigFloat!) {
  updateProductById(input: {id: $id, productPatch: {status: $status, updateBy: $currUser}}) {
    product {
      nodeId
      id
      productName
  ...
  render() {
    ...
    function onAfterInsertRow(row) {
      ...
      client.mutate({mutation: ADD_OPTION,
        variables:{keyName: row.productName, currUser: CURRENT_USER.currentUser.id, status: row.status }}).then(
          data => {
            console.log(data)
              if (data){
    ...
    function productTypeName (cell, row){
      return (<span>{row && row.productTypeByProductTypeId.productTypeName}</span>)
    }

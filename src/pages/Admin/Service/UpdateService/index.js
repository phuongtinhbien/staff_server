...
const SERVICE_QUERY = gql`query option($nodeId: BigFloat!){
  ...
  allUnitPrices (condition: {serviceTypeId: $nodeId, status: "ACTIVE"}){
    nodes{
      price
      productByProductId{
        productName
...
const CREATE_CLOTH = gql`mutation updateServiceType ($s:ServiceTypeInput!, $u: [UnitPriceInput!]) {
  updateServiceTypeAndUnitPrice(input:{
    s:$s,
    u:$u
  }){
    serviceType{
                ...
                {({loading, error,data, refetch}) => {
                  ...
                  if (data!= null){
                    ...
                    <BranchForm history={history} allProduct= {data.allProducts} onSubmit={value => handleSubmit(value,createNewBranch, CURRENT_USER.currentUser.id)} 
                              ...
                              allUnitPrice = {data.allUnitPrices.nodes}
                              serviceType = {data.serviceTypeById}
                              ></BranchForm>

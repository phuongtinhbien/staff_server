...
const UPDATE_WASHER_STATUS = gql `mutation updateOptionList($id: BigFloat!, $status: String!, $currUser: BigFloat!) {
  updateColorGroupById(input: {id: $id, colorGroupPatch: {status: $status, updateBy: $currUser}}) {
    colorGroup {
      nodeId
      id
      colorGroupName
...
mutation addOption ($keyName: String!, $status: String!,
  $currUser: BigFloat!){
  createColorGroup (input:{
    colorGroup:{
      colorGroupName: $keyName,
      status: $status,
      createBy: $currUser,
    }
  }){
  ...
  render() {
    ...
    function onAfterInsertRow(row) {
      ...
      client.mutate({mutation: ADD_OPTION,
        variables:{keyName: row.colorGroupName, currUser: CURRENT_USER.currentUser.id, status: row.status }}).then(
          data => {
            console.log(data)
              if (data){

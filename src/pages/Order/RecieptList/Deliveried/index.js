...
class ReceiptDeliveried extends Component {
  ...
  render() {
    ...
    return (
      ...
      variables = {{taskType:"TASK_RECEIPT",status: ["DELIVERIED"], branch: CURRENT_USER.branch.id }}
    ...
    >{({ loading, error, data, refetch }) => {
      ...
      return (
        <ReceiptTable tableName="Deliveried Receipts" tableDesc="All the list of deliveried receipts" orderList={proccessData(data.allTasks.nodes)}></ReceiptTable>
      );
      }
    }}
    </Query>
...
export default withRouter(ReceiptDeliveried);

...
const UPDATE_WASHER_STATUS = gql `mutation updateOptionList($id: BigFloat!, $status: String!, $currUser: BigFloat!) {
  updateServiceTypeById(input: {id: $id, serviceTypePatch: {status: $status, updateBy: $currUser}}) {
    serviceType {
      nodeId
      id
      serviceTypeName
...
mutation addOption ($keyName: String!, $status: String!, $desc: String!,
  $currUser: BigFloat!){
  createServiceType (input:{
    serviceType:{
      serviceTypeName: $keyName,
      status: $status,
      serviceTypeDesc: $desc,
      createBy: $currUser,
    }
  }){
    serviceType{
...
class TableDetail extends Component {
  ...
  state = {
    column: [],
    tableName: "Dịch vụ",
    tableDesc: "các dịch vụ hệ thống hỗ trợ",
  ...
  render() {
    ...
    function onAfterInsertRow(row) {
      ...
      client.mutate({mutation: ADD_OPTION,
        variables:{keyName: row.serviceTypeName, desc: row.serviceTypeDesc, currUser: CURRENT_USER.currentUser.id, status: row.status }}).then(
          data => {
            console.log(data)
              if (data){

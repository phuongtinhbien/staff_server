...
class OrderProcessing extends Component {
  ...
  render() {
    ...
    return (
      ...
      variables = {{taskType:"TASK_CUSTOMER_ORDER",status: ["FINISHED_SERVING"], branch: CURRENT_USER.branch.id }}
    ...
    >{({ loading, error, data, refetch, }) => {
      ...
      if (data != null){
      return (
        <OrderTable tableName="Đơn hàng đã xử lí" 
        tableDesc="Bao gồm các đơn hàng đã xử lí xong. Đang chờ được trả đồ hoặc tạo hóa đơn" 

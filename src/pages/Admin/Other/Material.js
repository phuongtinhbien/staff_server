...
const UPDATE_WASHER_STATUS = gql `mutation updateOptionList($id: BigFloat!, $status: String!, $currUser: BigFloat!) {
  updateMaterialById(input: {id: $id, materialPatch: {status: $status, updateBy: $currUser}}) {
    material {
      nodeId
      id
      materialName
  ...
  render() {
    ...
    function onAfterInsertRow(row) {
      ...
      client.mutate({mutation: ADD_OPTION,
        variables:{keyName: row.materialName, currUser: CURRENT_USER.currentUser.id, status: row.status }}).then(
          data => {
            console.log(data)
              if (data){
